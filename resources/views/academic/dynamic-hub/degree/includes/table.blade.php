                        ...
                        @if (isset($data['customer-status']) && $data['customer-status']->count() > 0)
                            ...
                            @foreach($data['customer-status'] as $customer_status)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button data-toggle="dropdown" class="btn-primary btn-sm dropdown-toggle {{ $customer_status->status == 'active'?"btn-info":"btn-warning" }}" >
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a class="btn-success btn-sm" href="{{ route($base_route.'.edit', ['id' => encrypt($customer_status->id)]) }}">
                                                <i class="ace-icon fa fa-pencil bigger-130" title="Edit"></i>
                                            </a>

                                            <a href="{{ route($base_route.'.delete', ['id' => encrypt($customer_status->id)]) }}" class="btn-danger btn-sm bootbox-confirm">

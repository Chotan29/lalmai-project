                        ...
                        @if (isset($data['download']) && $data['download']->count() > 0)
                            ...
                            @foreach($data['download'] as $download)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button data-toggle="dropdown" class="btn-primary btn-sm dropdown-toggle {{ $download->status == 'active'?"btn-info":"btn-warning" }}" >
                                            ...
                                            <ul class="dropdown-menu">
                                                <li>
                                                    <a href="{{ route($base_route.'.active', ['id' => encrypt($download->id)]) }}"><i class="fa fa-check btn-primary" aria-hidden="true"></i> Active</a>
                                                </li>

                                                <li>
                                                    <a href="{{ route($base_route.'.in-active', ['id' => encrypt($download->id)]) }}"><i class="fa fa-remove btn-warning" aria-hidden="true"></i> In-Active</a>
                                                </li>
                                            </ul>
                                        </div>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <a href="{{ asset('downloads'.DIRECTORY_SEPARATOR.$download->file) }}" target="_blank" class="btn-primary btn-sm">
                                            ...
                                            <a href="{{ route($base_route.'.edit', ['id' => encrypt($download->id)]) }}" class="btn-success btn-sm">
                                                <i class="ace-icon fa fa-pencil bigger-130" title="Edit"></i>
                                            </a>

                                            <a href="{{ route($base_route.'.delete', ['id' => encrypt($download->id)]) }}" class="btn-danger btn-sm bootbox-confirm" >

...
@section('css')
    ...
    <style>
        ...
        /* Centered custom modal (auto-sizes to content) */
        .modal-overlay{
        ...
        .modal{
          ...
          transform: scale(.98); animation: pop .18s ease-out forwards;
          position: relative;
        }
        ...
        .modal-toolbar{
          ...
          border-bottom:1px solid var(--border); flex-wrap:wrap; width:100%;
        }
        ...
        .child-grid{
          ...
          gap:12px;
          width:max-content;          /* grow to fit cards */
          max-width:60vw;             /* but cap inside viewport */
        }
        .child-card{
          background:#fff;border:1px solid #eef2f7;border-radius:14px;padding:14px;text-align:center;
          box-shadow:0 6px 18px rgba(2,8,23,.06);transition:.2s;text-decoration:none;color:inherit;cursor:pointer
        }
...
@section('js')
    {{-- Keep these if used elsewhere. Modal below is vanilla JS. --}}
    ...
    <script>
      ...
      function normalizeList(list){
        ...
        if (typeof list==='object') return Object.values(list); // handle associative
        return [];
      }
      const pick = (v,f) => v?.[f] ?? v?.[`data_${f}`] ?? '';
      ...
      function renderCards(items, paletteColor){
        ...
        const filtered = list.filter(n => {
          const hay = `${nodeTitle(n)} ${nodeDesc(n)} ${pick(n,'route_display')} ${pick(n,'route')}`.toLowerCase();

           ...
           ?>
                ...
                @if (isset($data['schedule_exams']) && $data['schedule_exams']->count() > 0)
                    ...
                    @foreach($data['schedule_exams'] as $exam)
                                {{-- hidden-print sits on the button block below, not here: the
                                     result itself has to survive a browser print of the profile. --}}
                                <div id="accordion{{$i}}" class="accordion-style1 panel-group">
                                    <div class="panel panel-default">
                                        ...
                                        <div class="panel-collapse collapse" id="collapseOne-{{$i}}" aria-expanded="false" style="height: 0px;">
                                            <div class="panel-body">
                                                ...
                                                @php($examTab = isset($data['exam_results'][$resultKey]) ? $data['exam_results'][$resultKey] : null)

                                                @if($examTab)
                                                    @php($result = $examTab['student']->first())
                                                    @php($cellCount = $examTab['tabulation_cell_count'] ?? (count($examTab['subject_columns']) * 2 + 4))
                                                    @php($density = $cellCount > 55 ? 'is-tight' : ($cellCount > 35 ? 'is-wide' : ''))

                                                    {{-- The student's own row, drawn with the class tabulation sheet's own markup
                                                         so the profile and the printed sheet can never look or read differently. --}}
                                                    <div class="tab-sheet-wrapper {{ $density }}" style="margin-bottom:14px;">
                                                        <div class="tab-one-head">
                                                            <strong>Tabulation</strong>
                                                            <span>
                                                                @if(isset($examTab['tabulation_published']) && $examTab['tabulation_published'])
                                                                    <span class="label label-success">Published to student</span>
                                                                @else
                                                                    <span class="label label-default">Not published</span>
                                                                @endif
                                                                <span class="tab-one-gpa {{ $result->gpa_remark === 'Pass' ? 'is-pass' : 'is-fail' }}">
                                                        ...
                                                        <div class="tab-sheet-scroll">
                                                            @include('print.exam.includes.tabulation-table', ['data' => $examTab, 'compact' => true])
                                                        </div>
                                                    </div>
                                                @else
                                                ...
                                                <div class="easy-link-menu align-center hidden-print">

                    ...
                    @foreach($data['books'] as $books)
                        <tr>
                            ...
                            <td>
                                ...
                                @if($availableCount > 0)
                                    <button data-toggle="dropdown" class="btn-primary btn-sm dropdown-toggle btn-success" >
                                        Available-{{ $books->bookCollection()->where('book_status','=',1)->count() }}
                                    </button>
                                @else
                                    <button data-toggle="dropdown" class="btn-primary btn-sm dropdown-toggle btn-warning" >
                                        Not Available
                                    </button>
                                @endif
                            ...
                            <td>
                                <div class="action-buttons">
                                    @if(!in_array($books->id,$data['book_request_ids']))
                                        <a href="{{ route('user-student.library.request-book', ['id' => encrypt($books->id)]) }}" class="btn-primary btn-sm">
                                            <i class="ace-icon fa fa-arrow-left bigger-130"></i> Request
                                        </a>
                                    @else
                                        {{--<button data-toggle="dropdown" class="btn-success btn-sm dropdown-toggle btn-success" >
                                            Requested
                                        </button>--}}
                                        <a href="#" class="btn-success btn-sm">
                                            <i class="ace-icon fa fa-check bigger-130"></i> Requested
                                        </a>
                                        <a href="{{ route('library.request-cancel', ['id' => encrypt($books->id),'member' => encrypt($data['lib_member']->id)]) }}" class="btn-danger btn-sm">

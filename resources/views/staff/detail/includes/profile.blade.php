<div class="row">
    <div class="col-sm-12 align-right hidden-print">
        <a href="{{ route($base_route.'.edit', ['id' => encrypt($data['staff']->id)]) }}" class="btn-primary btn-sm" >
    ...
    <div class="col-xs-12 col-sm-3 center">
        ...
        <div class="qr-code-container hidden-print">
                @if($data['staff']->reg_no != '')
                    {!! QrCode::size(150)->generate($data['staff']->reg_no) !!}
                    <div class="qr-code-label">staff ID QR Code</div>
                @endif
            </div>
            @if($data['staff']->reg_no != '' && class_exists('DNS1D'))
                <div class="qr-code-container hidden-print">
                    {!! DNS1D::getBarcodeSVG($data['staff']->reg_no, 'PHARMA2T') !!}
                    <div class="qr-code-label">Staff ID Barcode</div>
                </div>
            @endif
    </div>
...
<div class="row">
    <div class="space-6"></div>
    <div class="label label-info label-xlg arrowed-in arrowed-right arrowed">National ID's</div>
    <div class="space-6"></div>
    <div class="profile-user-info profile-user-info-striped">
        <div class="profile-info-row">
            @if($data['staff']->national_id_1 !="")
                <div class="profile-info-name"> ID 1. : </div>
                <div class="profile-info-value">
                    <span class="editable">{{ $data['staff']->national_id_1 }}</span>
                </div>
            @endif
            @if($data['staff']->national_id_2 !="")
                <div class="profile-info-name"> PAN : </div>
                <div class="profile-info-value">
                    <span class="editable">{{ $data['staff']->national_id_2 }}</span>
                </div>
            @endif
        </div>
    </div>
    <div class="profile-user-info profile-user-info-striped">
        <div class="profile-info-row">
            @if($data['staff']->national_id_3 !="")
                <div class="profile-info-name"> ID 3. : </div>
                <div class="profile-info-value">
                    <span class="editable">{{ $data['staff']->national_id_3 }}</span>
                </div>
            @endif
            @if($data['staff']->national_id_4 !="")
                <div class="profile-info-name"> ID 4 :</div>
                <div class="profile-info-value">
                    <span class="editable">{{ $data['staff']->national_id_4 }}</span>
                </div>
            @endif
        </div>
    </div>

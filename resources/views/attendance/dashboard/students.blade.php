...
@section('css')
    <style>
        ...
        * {
        ...
        /* Sticky header + filters in fullscreen */
        ...
        .container-fluid.is-fs .section--filters {
            position: sticky;
            top: var(--fs-header-h, 64px);
            z-index: 105;
            background: var(--primary-light);
            margin-top: 0;
            margin-bottom: 12px;
        ...
        /* Print: hide buttons / crisp charts handled in JS */
        @media print {
    ...
    <div class="container-fluid py-4">
        <div class="header">
            <div class="header-title">
                ...
                <p>Monitor and analyze student attendance patterns across faculties, semesters, and departments</p>
            </div>
            <div class="header-actions">
                <span class="chip" id="now-tz">--:--</span>
                <a href="{{ $students_index_route }}" class="btn btn-primary"><i class="fas fa-user-graduate"></i> Students</a>
                <a href="{{ $staff_index_route }}" class="btn btn-soft"><i class="fas fa-users"></i> Staff</a>
        ...
        <div class="chart-grid-2">
            <div class="section">
                <div class="section-head">
                    ...
                    <div class="title">Attendance by Department</div>
                </div>
                <div class="chart-box">
                    <div class="loading-overlay" id="dept-loading">
                        <div class="loading-spinner"></div>
                    </div>
                    <canvas id="ch-dept"></canvas>
                </div>
            </div>
            ...
            <div class="section">
                <div class="section-head">
                    ...
                    <div class="title">Attendance by Faculty</div>
                </div>
                <div class="chart-box">
                    <div class="loading-overlay" id="faculty-loading">
                        <div class="loading-spinner"></div>
                    </div>
                    <canvas id="ch-fac"></canvas>
                </div>
            </div>
            <div class="section">
                <div class="section-head">
                    <div class="icon"><i class="fas fa-layer-group"></i></div>
                    <div class="title">Attendance by Semester</div>
                </div>
                <div class="chart-box">
                    <div class="loading-overlay" id="semester-loading">
                        <div class="loading-spinner"></div>
                    </div>
                    <canvas id="ch-sem"></canvas>
                </div>
            </div>
            <div class="section">
                <div class="section-head">
                    <div class="icon"><i class="fas fa-user-group"></i></div>
                    <div class="title">Attendance by Batch</div>
                </div>
                <div class="chart-box">
                    <div class="loading-overlay" id="batch-loading">
                        <div class="loading-spinner"></div>
                    </div>
            ...
            <div class="section">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 15px;">
                    ...
                    <div class="stats-card">
                        ...
                        <div class="stats-content">
                            <div class="stats-title">Top Faculty</div>
                            <div class="stats-value" id="top-faculty">--</div>
                        </div>
                    </div>
                    <div class="stats-card">
                        <div class="stats-icon"><i class="fas fa-layer-group"></i></div>
                        <div class="stats-content">
                            <div class="stats-title">Top Semester</div>
                            <div class="stats-value" id="top-semester">--</div>
                        </div>
                    </div>
                    <div class="stats-card">
                        <div class="stats-icon"><i class="fas fa-user-group"></i></div>
                        <div class="stats-content">
                            <div class="stats-title">Top Batch</div>
                            <div class="stats-value" id="top-batch">--</div>
                        </div>
                    </div>
                    <div class="stats-card">
...
@section('js')
    ...
    <script>
        (function($) {
            const ENDPOINT = "{{ route('attendance.dashboard.students.summary') }}";

            let chTrend, chStatus, chGender, chFac, chSem, chBatch, chDept, chAge;
            let live = false,
                timer = null;

            function allCharts() {
                return [chTrend, chStatus, chGender, chFac, chSem, chBatch, chAge, chDept].filter(Boolean);
            }
            ...
            function initCharts() {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            backgroundColor: [
                                ...
                                'rgba(245, 158, 11, 0.8)',
                                'rgba(59, 130, 246, 0.8)', 'rgba(100, 116, 139, 0.8)',
                                'rgba(156, 163, 175, 0.8)'
                            ],
                            borderColor: [
                                'rgb(16,185,129)', 'rgb(239,68,68)', 'rgb(245,158,11)',
                                'rgb(59,130,246)', 'rgb(100,116,139)', 'rgb(156,163,175)'
                            ],
                            borderWidth: 1
                        }]
                    },
                    ...
                    return new Chart(el, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: 'Present',
                                data: [],
                                backgroundColor: 'rgba(52, 109, 165, 0.7)',
                                borderColor: 'rgba(52, 109, 165, 1)',
                                borderWidth: 1
                            }]
                        },
                ...
                chFac = yBar(document.getElementById('ch-fac'));
                chSem = yBar(document.getElementById('ch-sem'));
                chBatch = yBar(document.getElementById('ch-batch'));
                chAge = yBar(document.getElementById('ch-age'));
                chDept = yBar(document.getElementById('ch-dept'));
            }
            ...
            // Helper to ALWAYS update (or clear) a chart
            ...
            function refresh() {
                ...
                const p = {
                    ...
                    to_date: $('#to_date').val(),
                    _: Date.now() // cache-buster so manual date changes never cache
                };
                ...
                $.get(ENDPOINT, p).done(function(resp) {
                    ...
                    // Charts (ALWAYS set, even if empty arrays)
                    ...
                    setChartData(chFac, resp?.facultyWise?.labels, resp?.facultyWise?.data);
                    setChartData(chSem, resp?.semesterWise?.labels, resp?.semesterWise?.data);
                    setChartData(chBatch, resp?.batchWise?.labels, resp?.batchWise?.data);
                    setChartData(chAge, resp?.ageWise?.labels, resp?.ageWise?.data);
                    setChartData(chDept, resp?.departmentWise?.labels, resp?.departmentWise?.data);

                    // Top labels (reset to -- when no data)
                    $('#top-faculty').text((resp?.facultyWise?.labels || [])[0] ?? '--');
                    $('#top-semester').text((resp?.semesterWise?.labels || [])[0] ?? '--');
                    $('#top-batch').text((resp?.batchWise?.labels || [])[0] ?? '--');
                    $('#top-age').text((resp?.ageWise?.labels || [])[0] ?? '--');

                    // Stats
                    const total = Number(resp?.kpi?.total ?? 0);
                    const present = Number(resp?.kpi?.present ?? 0);
                    const dateRange = daysBetween($('#from_date').val(), $('#to_date').val());
                    const avgDaily = dateRange > 0 ? Math.round(present / dateRange) : 0;
                    $('#avg-daily').text(avgDaily);
                    const attendanceRate = total > 0 ? Math.round((present / total) * 100) : 0;
                    $('#attendance-rate').text(attendanceRate + '%');

                }).fail(function(xhr) {
                    console.error('students summary failed', xhr?.responseText || xhr?.statusText);
                }).always(function() {
                    hideLoading();
                });
            }
            ...
            function setFsHeaderHeight() {
                if (!isFullscreen()) {
                    $('.container-fluid').css('--fs-header-h', '');
                    return;
                }
            ...
            $(function() {
                ...
                // Auto-refresh on manual date change
                let debounceTimer = null;
                $('#from_date, #to_date').on('change input', function() {
                    $('.quick-filter-btn').removeClass('active');
                    clearTimeout(debounceTimer);
                    debounceTimer = setTimeout(refresh, 150); // tiny debounce to avoid double fire
                });
                ...
                $('#toggle-live').on('click', function() {
                    ...
                    $(this).toggleClass('active', live)
                        .html(
                            `<i class="${live?'fa fa-spinner fa-spin':'fas fa-broadcast-tower'}"></i> Live: ${live?'ON':'OFF'}`
                            );
                    if (live) {
                        refresh();
                ...
                $('#btn-fullscreen').on('click', function(e) {
                    e.preventDefault();
                    const target = document.querySelector('.container-fluid') || document
                        .documentElement;
                ...
                window.addEventListener('resize', function() {
                    if (isFullscreen()) setFsHeaderHeight();
                });
                ...
                window.addEventListener('beforeprint', () => setChartHiDPI(2.5));
                window.addEventListener('afterprint', () => resetChartSize());
                const mq = window.matchMedia && window.matchMedia('print');
                if (mq && mq.addEventListener) {
                    mq.addEventListener('change', e => {
                        if (e.matches) setChartHiDPI(2.5);
                        else resetChartSize();
                    });
                }
            });

...
<div class="container-fluid">
  <div class="header">
    ...
    <div class="controls">
      ...
      <a class="btn btn-outline" href="{{ route('attendance.live.students') }}"><i class="fas fa-user-graduate"></i> Students Page</a>
...
(function(){
  ...
  ['pointerdown','keydown','touchstart'].forEach(ev=>document.addEventListener(ev,()=>{ if(!acReady) ensureAC(); },{once:true}));
  function beep(freq=980, dur=.08, type='triangle', gain=.08){ if(!beepOn) return; ensureAC(); if(!ac) return; var o=ac.createOscillator(), g=ac.createGain(); o.type=type; o.frequency.value=freq; g.gain.value=gain; o.connect(g); g.connect(ac.destination); var t=ac.currentTime; o.start(t); o.stop(t+dur); }
  ...
  ['pointerdown','keydown','touchstart'].forEach(ev=>document.addEventListener(ev,()=>initTTS(),{once:true}));
  initTTS();
  ...
  function renderRow(item){
    ...
    var selected = state.sel_staff.has(pid) ? 'checked' : '';
  ...
  function loadList(){
    $.get(ROUTE_LIST, listParams()).done(function(resp){
      ...
      $('#staff-list').html('<div class="empty">Failed to load. Click Refresh.</div>');
    });
  }

  /* ---------- Selection ---------- */
  function updateSelCounts(){
    var c = state.sel_staff.size;
    $('#sel-count-staff').text(c+' selected');
    $('#sel-all-staff').prop('checked', c>0 && c === state.last_list.length);
  }
  function clearSelection(){
    state.sel_staff.clear();
    $('#sel-all-staff').prop('checked', false);
  ...
  $(document).on('change', '.sel-row', function(){
    ...
    if ($(this).is(':checked')) { state.sel_staff.add(pid); }
    else { state.sel_staff.delete(pid); }
    updateSelCounts();
  });
  ...
  $(document).on('click', '.js-mark', function(e){
    ...
    $.post(url, { code: code })
      ...
      .fail(function(xhr){ tintRow($card, prev); Swal.fire('Mark failed', xhr.responseJSON?.message || 'Error','error'); });
  });
  ...
  $(document).on('click', '.js-qmark', function(e){
    ...
    $btn.prop('disabled', true);
    $.post(ROUTE_QMARK, { type: type, person_id: pid, code: code, date: TODAY })
      .always(function(){ $btn.prop('disabled', false); })
      .done(function(){ speakStatus(code, who); })
      .fail(function(xhr){ tintRow($card, prev); Swal.fire('Mark failed', xhr.responseJSON?.message || 'Error','error'); });
  });
  ...
  $(document).on('click', '.js-check', function(e){
    ...
    $.post(url, {})
      .always(function(){ $btn.prop('disabled', false); })
      .fail(function(xhr){ Swal.fire('Check failed', xhr.responseJSON?.message || 'Error','error'); });
  });
  ...
  $(document).on('click', '.js-bulk', function(e){
    ...
    var ids  = Array.from(state.sel_staff);

    if (!ids.length) { noSelectionAlert(); return; }
    ...
    Swal.fire({
      ...
      html: `
        <div class="swal-custom-alert alert-warning">
          <i class="fa fa-exclamation-triangle mr-2"></i>
          You are about to perform: <strong>${label}</strong>
        </div>
      ...
      if (kind === 'check') {
        beep();
        $.post(ROUTE_BULK_CHECK, { type: 'staff', person_ids: ids, date: TODAY })
          .done(function(){
            clearSelection(); // deselect after bulk
            speak('Bulk check complete for '+ids.length+' staff');
          })
          .fail(function(xhr){ Swal.fire('Bulk check failed', xhr.responseJSON?.message || 'Error','error'); });
      } else {
        ids.forEach(function(pid){ tintRow($('.rowcard[data-pid="'+pid+'"]'), code); });
        beep();
        $.post(ROUTE_BULK_MARK, { type: 'staff', person_ids: ids, code: code, date: TODAY })
          .done(function(){
            clearSelection(); // deselect after bulk
            speak(statusWord(code)+' set for '+ids.length+' staff');
          })
          .fail(function(xhr){

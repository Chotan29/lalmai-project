...
@section('css')
<style>
:root{--primary:#346da5;--primary-light:#eef2ff;--secondary:#64748b;--success:#10b981;--warning:#f59e0b;--danger:#ef4444;--dark:#1e293b;--light:#f8fafc;--border:#e2e8f0;--shadow:0 4px 6px -1px rgba(0,0,0,.1),0 2px 4px -1px rgba(0,0,0,.06)}
...
.rowcard{display:flex;gap:12px;align-items:flex-start;border:1px solid var(--border);border-left:4px solid #cbd5e1;background:#f8fafc;border-radius:12px;padding:12px}
...
(function(){
  ...
  function setActivePill(code){
    qsa('.pill').forEach(function(p){ p.classList.remove('active'); });
  ...
  function renderAvatar(imgUrl, name){
  ...
  function renderRow(item){
    ...
    return ''+
    '<div class="rowcard" data-attendance="'+(attendanceId||'')+'" data-type="'+personType+'" data-pid="'+pid+'">'+
      '<div><input type="checkbox" class="sel-row" '+selected+'></div>'+
      renderAvatar(imgUrl, name)+
      '<div class="meta">'+
        '<h4>'+ name +'</h4>'+
  ...
  function loadList(){
    ...
    $.get(ROUTE_LIST, params).done(function(resp){
      ...
      if (subjMsg){ subjMsg.style.display='none'; subjMsg.textContent=''; }

      if (resp && resp.meta && resp.meta.need_hierarchy && state.type==='students'){
        applyList('#students-list', [], '#students-empty');
        document.getElementById('students-empty').textContent = 'Select Department → Faculty → Semester → Batch.';
        setMeta();
        return;
      }
  ...
  $(document).on('click', '.js-qmark', function(e){
    e.preventDefault();
    var $btn = $(this), type = $btn.data('type'), pid = $btn.data('pid'), code = $btn.data('code');
    $btn.prop('disabled', true);
  ...
  $(document).on('click', '.js-bulk', function(){
    ...
    if (kind === 'check') {
      $.post(@json(route('attendance.bulk.check')), { type: (state.type==='students'?'student':'staff'), person_ids: ids, date: TODAY })
        .done(function(){ state.sel_students.clear(); state.sel_staff.clear(); loadList(); })
        .fail(function(xhr){ alert('Bulk check failed: ' + (xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Error')); });
    } else {
      $.post(@json(route('attendance.bulk.mark')), { type: (state.type==='students'?'student':'staff'), person_ids: ids, code: code, date: TODAY, subject_id: (state.subj_id || '') })
        .done(function(){ state.sel_students.clear(); state.sel_staff.clear(); loadList(); })

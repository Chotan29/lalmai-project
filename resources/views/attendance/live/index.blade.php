...
/* SweetAlert custom look (from your sample) */
.swal-custom-popup{width:500px;max-width:95%;border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,0.15);padding:1.5rem}
.swal-custom-header{padding:0 0 1rem 0;border:none}
.swal-custom-title{font-weight:600;font-size:1.25rem;color:#343a40}
.swal-custom-content{padding:0;font-size:1rem}
.swal-custom-actions{margin:1.5rem 0 0 0;padding:0;justify-content:center}
.swal-custom-confirm-btn,.swal-custom-cancel-btn{padding:.5rem 1.25rem;font-size:.9rem;border-radius:4px;font-weight:500}
.swal-custom-alert{padding:.75rem 1rem;border-radius:4px;margin-bottom:0;border-left:4px solid transparent}
.alert-warning{background:#fff3cd;border-left-color:#ffc107;color:#856404}
.alert-light{background:#f8f9fa;border-left-color:#e9ecef;color:#495057}
.swal2-loader{border-color:#dc3545 transparent #dc3545 transparent !important}
</style>
@endsection
...
<div class="container-fluid">
  ...
  <div class="section">
    ...
    <div class="controls" style="margin-bottom:10px">
      ...
      {{-- STATUS FILTERS - give them filter-pill class and group wrapper --}}
    ...
    {{-- Student hierarchy (hidden on Staff tab) --}}
    ...
    <div id="hier-help" class="help" style="margin-top:6px">Select Department → Faculty → Semester → Batch to load students. If a Subject is chosen, it must be scheduled today to show students.</div>
  </div>
...
(function(){
  ...
  function pickVoice(){
    ...
    return vs.find(v=>/en(-|_)?(US|GB|IN)?/i.test(v.lang||'')) || vs.find(v=>/English/i.test(v.name||'')) || vs[0] || null;
  }
  function initTTS(){
  ...
  function speakStatus(code, name){
    if(!voiceOn) return;
    if(!('speechSynthesis' in window)) return;
    var text = statusWord(code) + (name ? (' for ' + name) : '');
    try{
  ...
  function setActiveFilterPill(code){
    ...
    var el=document.querySelector('.filter-pill[data-code="'+(code||'')+'"]');
  ...
  function setMeta(){
    ...
    var txt = (state.type==='students'?'Students':'Staff')+' · '+TODAY+(scope.length?(' · '+scope.join(' / ')):'' );
    (state.type==='students'?$('#students-meta'):$('#staff-meta')).text(txt);
  }
  ...
  function loadList(){
    ...
    $.get(ROUTE_LIST, params).done(function(resp){
      ...
      if (resp && resp.meta && resp.meta.need_hierarchy && state.type==='students'){
        ...
        $('#students-empty').text('Select Department → Faculty → Semester → Batch.');
        setMeta();
        return;
      }

      if (resp && resp.meta && resp.meta.subject_selected && resp.meta.subject_scheduled === false){
        subjMsg.text(resp.meta.subject_message || 'Subject is not scheduled today.').show();
        applyList('#students-list', [], '#students-empty');
        setMeta();
        return;
      }
      ...
      var target = (state.type==='students') ? '#students-list' : '#staff-list';
      $(target).html('<div class="empty">Failed to load. Click Refresh.</div>');
    });
  }
  ...
  $(document).on('click', '.js-mark', function(e){
    ...
    tintRow($card, code); beep();
    $btn.prop('disabled', true);
    $.post(url, { code: code, subject_id: (state.subj_id || '') })
      .always(function(){ $btn.prop('disabled', false); })
      .done(function(){ speakStatus(code, who); })
      .fail(function(xhr){ tintRow($card, prev); alert('Mark failed: ' + (xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Error')); });
  });
  ...
  $(document).on('click', '.js-qmark', function(e){
    ...
    var $card = $btn.closest('.rowcard'), who = $card.attr('data-name') || '', prev = ($card.attr('data-status')||'').toUpperCase();
    tintRow($card, code); beep();
    $btn.prop('disabled', true);
  ...
  function updateSelCounts(){
    ...
    (state.type==='students' ? $('#sel-all-students') : $('#sel-all-staff')).prop('checked', c>0 && c === state.last_list.length);
  }
  ...
  function noSelectionAlert(){
    return Swal.fire({
      title: '<i class="fa fa-exclamation-circle text-danger mr-2"></i> No items selected',
      html: `
        <div class="swal-custom-alert alert-warning">
          <i class="fa fa-exclamation-triangle mr-2"></i>
          Please select at least <strong>one</strong> row to perform a bulk action.
        </div>
        <div class="swal-custom-alert alert-light mt-3">
          <i class="fa fa-info-circle text-info mr-2"></i>
          Use the checkboxes on each row or "Select all on page".
        </div>
      `,
      icon: 'warning',
      confirmButtonText: '<i class="fa fa-check mr-2"></i> OK',
      customClass: {
  ...
  function confirmBulk(kindLabel, count){
    return Swal.fire({
      title: '<i class="fa fa-exclamation-circle text-danger mr-2"></i> Confirm Bulk Action',
      html: `
        <div class="swal-custom-alert alert-warning">
          <i class="fa fa-exclamation-triangle mr-2"></i>
          You are about to perform: <strong>${kindLabel}</strong>
        </div>
        <div class="swal-custom-alert alert-light mt-3">
          <i class="fa fa-info-circle text-info mr-2"></i>
          Target: <strong>${count}</strong> selected item(s).
        </div>
  ...
  $(document).on('click', '.js-bulk', function(e){
    e.preventDefault();
    e.stopPropagation(); // make sure it never bubbles to any filter handlers

    var kind = $(this).data('kind');                  // 'mark' or 'check'
    var code = String($(this).data('code')||'').toUpperCase(); // for 'mark'
    ...
    confirmBulk(label, ids.length).then(function(res){
      if (!res.isConfirmed) return;

      if (kind === 'check') {
        beep();
        $.post(ROUTE_BULK_CHECK, { type: (state.type==='students'?'student':'staff'), person_ids: ids, date: TODAY })
          .done(function(){ /* leave as-is */ })
        ...
        beep();
        speakStatus(code, ids.length + (state.type==='students'?' students':' staff'));
        $.post(ROUTE_BULK_MARK, { type: (state.type==='students'?'student':'staff'), person_ids: ids, code: code, date: TODAY, subject_id: (state.subj_id || '') })
          .fail(function(xhr){
            alert('Bulk mark failed: ' + (xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Error'));
            loadList(); // revert to server truth if needed
          });
      }
    });
  });

  /* ---------- Filters (STRICTLY on filter pills only) ---------- */
  ...
  document.querySelectorAll('.nav-tabs .nav-link').forEach(function(link){
    link.addEventListener('click', function(){
      ...
      var hier = document.getElementById('hier-section');
      var help = document.getElementById('hier-help');
      var subjMsg = document.getElementById('subject-msg');
      if (target === '#tab-students') { hier.style.display=''; help.style.display=''; }
      else { hier.style.display='none'; help.style.display='none'; subjMsg.style.display='none'; }

      state.type = (target==='#tab-staff') ? 'staff' : 'students';
      // clear selections when switching
      if (state.type==='students'){ state.sel_staff.clear(); } else { state.sel_students.clear(); }
      updateSelCounts();
      loadList();
    });
  });

...
a.link{color:#0f172a;text-decoration:none;border:1px solid var(--border);padding:6px 10px;border-radius:10px;background:#fff}
a.link:hover{background:#f1f5f9}
...
<div class="container-fluid py-3">
  ...
  <div class="header">
    <div>
      <h1>Live Attendance — Manual</h1>
      <div class="sub">Bulk mark, inline modify, hierarchy filters.</div>
    </div>
    <div class="controls">
      <a class="link" href="{{ route('attendance.live.scan') }}"><i class="fas fa-qrcode"></i> Scanner</a>
  ...
  <div class="section">
    ...
    <div id="hier-help" class="help" style="margin-top:6px">Select Department → Faculty → Semester → Batch. If a Subject is chosen and not scheduled, **Modify** mode still shows the already-marked rows.</div>
  </div>
...
(function(){
  ...
  function setMeta(){
    ...
    // pill highlight
    qsa('.pill.mode').forEach(function(p){ p.classList.remove('active'); });
    var mEl = document.querySelector('.pill.mode[data-mode="'+state.mode+'"]'); if (mEl) mEl.classList.add('active');
  }
  ...
  function afterAction(cardRef){
    state.mode = 'modify';          // keep rows visible for edit

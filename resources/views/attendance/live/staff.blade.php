...
(function(){
  ...
  initTTS();
  function statusWord(code){ var m={P:'Present',A:'Absent',L:'Late',E:'Excused',HL:'Half-Leave'}; var c=String(code||'').toUpperCase(); for(var i=0;i<STATUSES.length;i++){ if(String(STATUSES[i].code||'').toUpperCase()===c) return STATUSES[i].label||c; } return m[c]||c; }
  ...
  function applyList(where, list, emptySel){
    ...
    countsFromList(state.last_list);
    updateSelCounts();
  }

  function setMeta(){ $('#staff-meta').text('Staff · '+TODAY); }
  ...
  /* ---------- Alerts ---------- */
  function noSelectionAlert(){ return Swal.fire('No items selected','','warning'); }
  ...
  $(document).on('click', '.js-bulk', function(e){
    ...
    if (kind === 'check') {
      ...
      $.post(ROUTE_BULK_CHECK, { type: 'staff', person_ids: ids, date: TODAY })
        .done(function(){ clearSelection(); speak('Bulk check complete for '+ids.length+' staff'); })
      ...
      $.post(ROUTE_BULK_MARK, { type: 'staff', person_ids: ids, code: code, date: TODAY })
        .done(function(){ clearSelection(); speak(statusWord(code)+' set for '+ids.length+' staff'); })
        .fail(function(xhr){

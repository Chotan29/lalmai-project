...
<head>
    ...
    <title>Upay Payment Status</title>
    ...
    <style>
        ...
        .status-icon.failed, .status-icon.cancelled { background-color: rgba(220, 53, 69, 0.1); color: var(--failed); }

        .status-title { font-size: 2rem; margin-bottom: 15px; font-weight: 600; }
        .status-title.success { color: var(--success); }
        .status-title.pending { color: var(--pending); }
        .status-title.failed, .status-title.cancelled { color: var(--failed); }
        ...
        .status-badge.failed, .status-badge.cancelled { background-color: rgba(220, 53, 69, 0.1); color: var(--failed); }
    ...
    <div class="payment-status-container">
        <div class="payment-status-card">
            ...
            @else
                ...
                <p class="status-message">{{ $message ?? 'We couldn’t process your payment. Please try again or contact support.' }}</p>
            @endif
            ...
            <div class="transaction-details">
                ...
                @endif
                @if(isset($payment->payment_gateway))
                    <div class="detail-row">
            ...
            <div class="action-buttons">
                ...
                {{-- @if ($status === 'failed' || $status === 'cancelled')
                    <a href="{{ route('upay.payment.form') }}" class="btn btn-secondary">
            ...
            @if ($status === 'success' || $status === 'completed')
                <div style="margin-top: 30px; font-size: 0.9em; color: var(--gray);">
                    <p>A payment receipt has been sent to your email.</p>

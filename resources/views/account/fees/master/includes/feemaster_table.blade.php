                        ...
                        @foreach($data['fee_master'] as $fee_master)
                            <tr>
                                <td class="center first-child">
                                    <label>
                                        <input type="checkbox" name="chkIds[]" value="{{ encrypt($fee_master->id) }}" class="ace" />
                                        <span class="lbl"></span>
                                    </label>
                                </td>
                                <td>{{ $i }}</td>
                                <td> <a href="{{ route('student.view', ['id' => encrypt($fee_master->students_id)]) }}">{{ $fee_master->reg_no }}</a></td>
                                <td> <a href="{{ route('student.view', ['id' => encrypt($fee_master->students_id)]) }}">{{ $fee_master->first_name.' '.$fee_master->middle_name.' '. $fee_master->last_name }}</a></td>
                                ...
                                <td>
                                    @if($fee_master->feeCollect()->count() > 0)
                                        <div class="btn-primary btn-sm action-buttons">
                                            <a class="white" href="{{ route('account.fees.collection.view', ['id' => encrypt($fee_master->students_id)]) }}">
                                    ...
                                    @else
                                        ...
                                        <div class="btn-success btn-sm action-buttons">
                                            <a class="white" href="{{ route($base_route.'.edit', ['id' => encrypt($fee_master->id)]) }}">
                                                <i class="ace-icon fa fa-pencil bigger-130" title="Edit"></i>
                                            </a>
                                        </div>
                                        @ability('super-admin','fees-master-delete')
                                            <div class="btn-danger btn-sm action-buttons">
                                                <a class="white bootbox-confirm"  href="{{ route($base_route.'.delete', ['id' => encrypt($fee_master->id)]) }}">

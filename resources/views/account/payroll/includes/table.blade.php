            ...
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                    ...
                    @if (isset($data['salaryPay']) && $data['salaryPay']->count() > 0)
                        ...
                        @foreach($data['salaryPay'] as $salaryPay)
                            <tr>
                                <td>{{ $i }}</td>
                                <td> <a href="{{ route('staff.view', ['id' => encrypt($salaryPay->staff_id)]) }}" target="_blank">{{ $salaryPay->reg_no }}</a></td>
                                <td> <a href="{{ route('staff.view', ['id' => encrypt($salaryPay->staff_id)]) }}" target="_blank">{{ $salaryPay->first_name.' '.$salaryPay->middle_name.' '. $salaryPay->last_name }}</a></td>
                                ...
                                <td>{{ $salaryPay->payment_method }}</td>
                                <td> {{  ViewHelper::getUserNameId( $salaryPay->created_by ) }}</td>
                                <td>
                                    <div class="btn-primary btn-sm action-buttons">
                                        <a class="white" href="{{ route('account.salary.payment.view', ['id' => encrypt($salaryPay->staff_id)]) }}">

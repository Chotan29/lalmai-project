            ...
            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                ...
                @if (isset($data['staff']) && $data['staff']->count() > 0)
                    ...
                    @foreach($data['staff'] as $staff)
                        <tr>
                            <td>{{ $i }}</td>
                            <td><a href="{{ route('staff.view', ['id' => encrypt($staff->id)]) }}" target="_blank">{{ $staff->reg_no }}</a></td>
                            <td><a href="{{ route('staff.view', ['id' => encrypt($staff->id)]) }}" target="_blank"> {{ $staff->first_name.' '.$staff->middle_name.' '. $staff->last_name }}</a></td>
                            ...
                            <td>
                                <div class="btn-primary btn-sm action-buttons ">
                                    <a class="white" href="{{ route($base_route.'.view', ['id' => encrypt($staff->id)]) }}" target="_blank">

...
use App\Http\Requests\Certificate\InternCompletion\AddValidation;
use App\Http\Requests\Certificate\InternCompletion\EditValidation;
use App\Models\CourseCompletionCertificate;
use App\Models\CertificateHistory;
use App\Models\CertificateTemplate;
use App\Models\InternCompletionCertificate;
use App\Models\Student;
use Carbon\Carbon;
use Illuminate\Http\Request;
use URL;
use ViewHelper;
class InternCompletionCertificateController extends CollegeBaseController
{
    protected $base_route = 'certificate.intern-completion';
    protected $view_path = 'certificate.intern-completion';
    protected $panel = 'Intern Completion Certificate';
    ...
    public function index(Request $request)
    {
        ...
        $data['student'] = Student::select('students.id','students.reg_no', 'students.reg_date',
            ...
            'students.middle_name', 'students.last_name', 'ic.id as certificate_id','ic.date_of_issue', 'ic.internship_title',
            'ic.period', 'ic.character', 'ic.ref_text')
            ->where(function ($query) use ($request) {

                $this->commonStudentFilterCondition($query, $request);

                if ($request->has('issue-start-date') && $request->has('issue-end-date')) {
                    $query->whereBetween('ic.date_of_issue', [$request->get('issue-start-date'), $request->get('issue-end-date')]);
                    ...
                    $query->where('ic.date_of_issue', '>=', $request->get('issue-start-date'));
                    $this->filter_query['issue-start-date'] = $request->get('issue-start-date');
                } elseif ($request->has('issue-end-date')) {
                    $query->where('ic.date_of_issue', '<=', $request->get('issue-end-date'));
                    $this->filter_query['issue-end-date'] = $request->get('issue-end-date');
                }

                if ($request->has('character')) {
                    $query->where('ic.character', 'like', '%'. $request->character.'%');
                    $this->filter_query['ic.character'] = $request->character;
                }

                if ($request->has('period')) {
                    $query->where('ic.period', 'like', '%'. $request->period.'%');
                    $this->filter_query['ic.period'] = $request->period;
                }

                if ($request->has('intern')) {
                    $query->where('ic.internship_title', 'like', '%'. $request->intern.'%');
                    $this->filter_query['ic.internship_title'] = $request->intern;
                }
    ...
    public function add(Request $request, $id)
    {
        ...
        $certificateExist = InternCompletionCertificate::where('students_id',$id)->first();
        if($certificateExist){
    ...
    public function store(AddValidation $request)
    {
        ...
        $row = InternCompletionCertificate::create($request->all());

        if($row){
            $CreateHistory = CertificateHistory::create([
                'students_id' => $row->students_id,
                'certificate' => 'intern-completion',
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        $row = $student->internCompletionCertificate()->update($updateData);
        ...
        if($row){
            ...
            $CreateHistory = CertificateHistory::create([
                ...
                'certificate' => 'intern-completion',
                'certificate_id' => $student->internCompletionCertificate()->first()->id,
    ...
    public function delete(Request $request, $id)
    {
        ...
        $certificate = $row->internCompletionCertificate()->first();
        //delete history
        ...
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['print','active', 'in-active', 'delete'])) {
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    $row = InternCompletionCertificate::find($row_id);
                    switch ($request->get('bulk_action')) {
                        case 'print':
                            $data['certificate_template'] = $certificateTemplate = CertificateTemplate::where('certificate','=','INTERNSHIP COMPLETION')
                                ->first();
                            $data['student'] = $this->bulkPrint($certificateTemplate, $request->get('stuIds'));
                            return view(parent::loadDataToView('print.certificate.intern-completion'), compact('data'));
                            break;
                        case 'active':
                        case 'in-active':
                            if ($row) {

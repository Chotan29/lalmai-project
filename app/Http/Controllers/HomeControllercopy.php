...
class HomeControllercopy extends CollegeBaseController
{
    use StudentScopes;
    ...
    protected function buildMenuGroups($generalSetting): array
    {
        ...
        foreach ($menuGroups as $group) {
            ...
            foreach ($group['items'] as $item) {
                ...
                } elseif (isset($item['url']) && $item['external']) {
                    $items[] = [
                        'title' => $item['title'],
                        'desc' => $item['desc'] ?? '',
                        'icon' => $item['icon'] ?? 'fa-link',
    ...
    public function welcome()
    {
        if (Auth::check()) {
            ...
            $data['welcome_menu'] = $data['menus'] ? $data['menus']->pages()->orderBy('rank')->get() : collect();

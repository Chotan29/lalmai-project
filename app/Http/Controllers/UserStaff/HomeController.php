...
class HomeController extends CollegeBaseController
{
    ...
    public function subject()
    {
        ...
        $id = auth()->user()->hook_id;
        $data = [];
        $data['subject'] = Subject::forTeacher($id)->orderBy('title')->get();
    ...
    public function attendance()
    {
        ...
        $id = (int) auth()->user()->hook_id;
        $data = [];

        $yearIds = Year::pluck('id', 'title')->toArray();
        $monthIds = Month::pluck('id', 'title')
            ->mapWithKeys(function ($id, $title) {
                return [strtolower($title) => $id];
            })->toArray();

        $staffTypes = [Staff::class, 'staff'];

        $data['attendance'] = Attendance::select('id', 'date', 'attendance_status_id')
            ->where('status', 1)
            ->whereIn('attendable_type', $staffTypes)
            ...
            ->map(function ($monthRows) use ($yearIds, $monthIds) {
                $firstDate = Carbon::parse($monthRows->first()->date);

                $row = [
                    'id' => $monthRows->first()->id,
                    'years_id' => $yearIds[$firstDate->format('Y')] ?? null,
                    'months_id' => $monthIds[strtolower($firstDate->format('F'))] ?? $firstDate->month,
                ];
                ...
                foreach ($monthRows as $attendance) {
                    $dayColumn = 'day_'.Carbon::parse($attendance->date)->day;
                    $row[$dayColumn] = $attendance->attendance_status_id;
                }

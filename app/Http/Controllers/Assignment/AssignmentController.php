...
class AssignmentController extends CollegeBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        $id = decrypt($id);
        $data = [];
        $data['row'] = Assignment::select('assignments.id','assignments.years_id','assignments.semesters_id','assignments.subjects_id','assignments.publish_date',
            'assignments.end_date', 'assignments.title','assignments.description','assignments.file', 'assignments.status',
            'f.id as faculty_id','f.faculty','sem.semester','sub.title as subject')
            ->where('assignments.id',$id)
            ->join('semesters as sem','sem.id','=','assignments.semesters_id')
            ->join('subjects as sub','sub.id','=','assignments.subjects_id')
            ->join('faculty_semester as fs','fs.semester_id','=','sem.id')
            ->join('faculties as f','f.id','=','fs.faculty_id')
            ->first();

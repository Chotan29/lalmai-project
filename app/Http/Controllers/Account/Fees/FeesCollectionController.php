...
class FeesCollectionController extends CollegeBaseController
{
    ...
    public function view(Request $request, $id)
    {
        try {
            ...
            //onlinePayment
            $data['onlinePayments'] = $data['student']->onlinePayments()->get();      
            ...
            // Some fee heads are excluded from the API installment payload.
            // Keep the ledger summary visible by calculating a local fallback.
            $installmentAmount = isset($data['current_unpaid_installment']['installmentAmount'])
                ? (float) $data['current_unpaid_installment']['installmentAmount']
                : 0;
            $balanceAmount = (float) $data['student']->balance;
            $hasAmountMismatch = abs($installmentAmount - $balanceAmount) > 0.009;
            ...
            if (
                ...
                $fallbackInstallment = $this->buildLedgerInstallmentFallback($data['student']);
            ...
            $request->session()->flash($this->message_warning, 'Invalid student ID or student not found.');
            return redirect()->route($this->base_route);
        }
    }

    protected function buildLedgerInstallmentFallback($student)
    {
        ...
        $paidAmount = round($feeMasters->sum(function ($feeMaster) {
            return $feeMaster->collections()->where('status', 1)->sum('paid_amount');
        }), 2);
        $discountAmount = round($feeMasters->sum(function ($feeMaster) {
            return $feeMaster->collections()->where('status', 1)->sum('discount');
        }), 2);
        $fineAmount = round($feeMasters->sum(function ($feeMaster) {
            return $feeMaster->collections()->where('status', 1)->sum('fine');
        }), 2);

        $dueDate = $feeMasters->min('fee_due_date') ?: ($latestFeeMaster->fee_due_date ?: now()->format('Y-m-d'));
        $currentPayableAmount = max(0, round(($totalAmount + $fineAmount) - ($paidAmount + $discountAmount), 2));
        $isOverdue = $currentPayableAmount > 0 && $currentDate->gt(Carbon::parse($dueDate));
        $status = $currentPayableAmount <= 0 ? 'paid' : ($isOverdue ? 'overdue' : 'pending');
        $installments = [[
            'number' => 1,
            'percentage' => 100,
            'initial_due_amount' => round($totalAmount + $fineAmount, 2),
            'due_date' => $dueDate,
            'status' => $status,
            ...
            'fine' => $fineAmount,
            'due_amount' => $currentPayableAmount,
        ]];
    ...
    public function delete(Request $request, $id)
    {
        ...
        //$request->session()->flash($this->message_success, $this->panel.' Cancel Successfully.');
        return back();
    }
    ...
    public function quickReceiveStore(Request $request)
    {
        ...
        $filtered  = $students->filter(function ($student) use ($request) {
         ...
         if($request->print_receipt == "detail"){
            return redirect()->route('print-out.fees.date-receipt-detail', ['id' => encrypt($request->students_id), 'date' => $request->date]);
        }

        if($request->print_receipt == "short"){
            return redirect()->route('print-out.fees.date-receipt', ['id' => encrypt($request->students_id), 'date' => $request->date]);
        }
    ...
    public function dueView(Request $request, $id)
    {
        ...
        // Keep collect-profile view safe for due flow by always providing installment payload keys.
        $data['current_unpaid_installment'] = ['installmentAmount' => 0];
        $data['current_installment_detail'] = [];

        $currentUnpaidInstallment = $this->currentUnpaidInstallment($data['student']->reg_no);
        if (is_array($currentUnpaidInstallment)) {
            $data['current_unpaid_installment'] = array_merge(['installmentAmount' => 0], $currentUnpaidInstallment);
        }

        $currentInstallmentDetail = $this->currentUnpaidInstallmentDetail($data['student']->reg_no);
        if (is_array($currentInstallmentDetail) && isset($currentInstallmentDetail['installments']) && is_array($currentInstallmentDetail['installments'])) {
            $data['current_installment_detail'] = $currentInstallmentDetail['installments'];
        }

        $installmentAmount = (float) ($data['current_unpaid_installment']['installmentAmount'] ?? 0);
        $balanceAmount = (float) $data['student']->balance;
        $hasAmountMismatch = abs($installmentAmount - $balanceAmount) > 0.009;

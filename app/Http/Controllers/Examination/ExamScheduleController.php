...
class ExamScheduleController extends CollegeBaseController
{
    ...
    protected function groupedScheduleQuery(array $group)
    {
        return ExamSchedule::where([
            ['years_id', '=', $group['year']],
            ['months_id', '=', $group['month']],
            ['exams_id', '=', $group['exam']],
            ['faculty_id', '=', $group['faculty']],
            ['semesters_id', '=', $group['semester']],
        ]);
    }

    protected function decodeBulkScheduleGroup($value)
    {
        $parts = explode('|', $value);
    ...
    public function bulkAction(Request $request)
    {
        if (!$request->has('bulk_action') || !in_array($request->get('bulk_action'), ['active', 'in-active', 'delete'])) {
            $request->session()->flash($this->message_warning, 'Invalid bulk action request.');
            return redirect()->route($this->base_route);
        }
        ...
        foreach ($request->get('chkIds') as $groupKey) {
            $group = $this->decodeBulkScheduleGroup($groupKey);
            if (!$group) {
                continue;
            }

            $scheduleIds = array_merge($scheduleIds, $this->groupedScheduleQuery($group)->pluck('id')->toArray());
        }

        $scheduleIds = array_values(array_unique($scheduleIds));

        if (count($scheduleIds) === 0) {
            $request->session()->flash($this->message_warning, 'No valid '.$this->panel.' selected.');
            return redirect()->route($this->base_route);
        }

        switch ($request->get('bulk_action')) {
            case 'active':
                ExamSchedule::whereIn('id', $scheduleIds)->update(['status' => 1]);
                $request->session()->flash($this->message_success, $this->panel.' Active Successfully.');
                break;
            case 'in-active':
                ExamSchedule::whereIn('id', $scheduleIds)->update(['status' => 0]);
                $request->session()->flash($this->message_success, $this->panel.' In-Active Successfully.');
                break;
            case 'delete':
                ExamSchedule::whereIn('id', $scheduleIds)->delete();
                $request->session()->flash($this->message_success, $this->panel.' Deleted Successfully.');
                break;
        }
    ...
    public function publish(Request $request, $year=null,$month=null,$exam=null,$faculty=null,$semester=null)
    {
        ...
        ExamSchedule::whereIn('id', $examIds)->update([
            'publish_status' => 1,
            'publish_date' => now(getenv('APP_TIMEZONE'))
        ]);

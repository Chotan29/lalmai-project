...
class SubjectController extends CollegeBaseController
{
    ...
    public function index()
    {
       ...
       $data['subject'] = Subject::orderBy('code')
        ...
        $data['staffs'] = [];
        foreach (Staff::select('id','reg_no','first_name','middle_name','last_name')->Active()->get() as $staff) {
            $data['staffs'][$staff->id] = trim($staff->first_name.' '.$staff->middle_name.' '.$staff->last_name).' ['.$staff->reg_no.']';
        }
    ...
    {
        $staffIds = array_values(array_filter((array) $request->get('staff_ids')));

        $request->request->add([
            'created_by' => auth()->user()->id,
            'staff_id' => count($staffIds) ? $staffIds[0] : null,
        ]);
        $subject = Subject::create($request->all());

        /*Attach all assigned teachers*/
        $subject->teachers()->sync($staffIds);
    ...
    public function edit($id)
    {
        ...
        $data['subject'] = Subject::select('id', 'title', 'code', 'short_name', 'full_mark_theory', 'pass_mark_theory',
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        /*Re-sync assigned teachers*/
        $row->teachers()->sync($staffIds);

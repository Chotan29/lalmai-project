...
class HomeController extends CollegeBaseController
{
    ...
    public function attendance(Request $request, $id)
    {
        ...
        //attendance start
        $attendanceCollection = Attendance::where('status', 1)
            ->where('attendable_id', $id)
            ->whereIn('attendable_type', [Student::class, 'student'])
            ->orderBy('date', 'asc')
            ->get();

        $attendanceCollection = Attendance::legacyMonthlyCollection($attendanceCollection, 1);
